...
export class InstanceManager {
  ...
  public register(connection: WebSocket, providedInstanceId?: string): string {
    ...
    if (providedInstanceId && this.instances.has(providedInstanceId)) {
      ...
      // 关闭旧连接
      if (existing.connection !== connection && existing.connection.readyState === existing.connection.OPEN) {
    ...
    console.log(`${LOG_PREFIX} 实例注册: ${instanceId} (总计: ${this.instances.size})`);
    return instanceId;
  }
